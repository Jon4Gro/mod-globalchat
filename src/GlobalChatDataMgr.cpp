...
void GlobalChatMgr::SendGlobalChat(WorldSession* session, std::string_view message, TeamId toTeam)
{
    ...
    {
        ChatHandler(session).SendSysMessage("|cffff0000GlobalChat is currently disabled.|r");
        return;
    }

    if (playersChatData[guid].IsBanned())
    {
        ChatHandler(session).SendSysMessage("|cffff0000You are currently banned from the GlobalChat.|r");
        return;
    }
    ...
    if (!player->CanSpeak() || playersChatData[guid].GetMuteTime() > GameTime::GetGameTime().count())
    {
        ...
        ChatHandler(session).SendSysMessage(Acore::StringFormat("|cffff0000You can't use the GlobalChat while muted.|r You need to wait another {}.", secsToTimeString(muteLeft)));
        return;
    }

    if (!IsInChat(guid))
    {
        ChatHandler(session).SendSysMessage("|cffff0000You have not joined the GlobalChat. Type |r.joinglobal|cffff0000 to join the GlobalChat.|r");
        return;
    }

    if (chatText.empty())
    {
        ChatHandler(session).SendSysMessage("Your message cannot be empty.");
        return;
    }

    if (ProfanityBlockType > 0 && playerSecurity <= ProfanityBlockLevel && HasForbiddenPhrase(chatText))
    {
        ...
        {
            ChatHandler(session).SendSysMessage("Your message contains a forbidden phrase.");
        ...
        if (ProfanityBlockType == 3)
        {
            if (ProfanityMute > 0)
            {
                ...
                ChatHandler(session).SendSysMessage(Acore::StringFormat("Your message contains a forbidden phrase. You have been muted for {}.", secsToTimeString(ProfanityMute)));
    ...
    if (URLBlockType > 0 && playerSecurity <= URLBlockLevel && HasForbiddenURL(chatText))
    {
        if (URLBlockType == 1)
        {
        ...
        {
            ChatHandler(session).SendSysMessage("Urls are not allowed.");
        ...
        if (URLBlockType == 3)
        {
            if (URLMute > 0)
            {
                ...
                ChatHandler(session).SendSysMessage(Acore::StringFormat("Urls are not allowed. You have been muted for {}.", secsToTimeString(URLMute)));
    ...
    if (player->GetTotalPlayedTime() <= MinPlayTime && session->GetSecurity() == 0)
    {
        ...
        ChatHandler(session).SendSysMessage(Acore::StringFormat("You must have played at least {} to use the GlobalChat. {} remaining.", minTime, adStr));
        return;
    }
...
void GlobalChatMgr::PlayerJoinCommand(ChatHandler* handler)
{
    ...
    {
        handler->SendSysMessage("The GlobalChat is currently disabled.");
        return;
    }

    if (IsInChat(guid))
    {
        handler->SendSysMessage("You already joined the GlobalChat.");
        return;
    }

    playersChatData[guid].SetInChat(true);

    handler->SendSysMessage("You have joined the GlobalChat.");
...
void GlobalChatMgr::PlayerLeaveCommand(ChatHandler* handler)
{
    ...
    if (!IsInChat(guid))
    {
        handler->SendSysMessage("You already left the GlobalChat.");
        return;
    }

    playersChatData[guid].SetInChat(false);
    handler->SendSysMessage("You have left the GlobalChat.");
...
void GlobalChatMgr::PlayerInfoCommand(ChatHandler* handler, Player* player)
{
    ...
    handler->SendSysMessage(Acore::StringFormat("GlobalChat information about player |cff4CFF00{}|r", player->GetName()));
    handler->SendSysMessage(Acore::StringFormat("> In Chat: {} || Last Message: {} ", inChat ? "|cff4CFF00Yes|r" : "|cffFF0000No|r", lastMessage ? ("|cff4CFF00" + lastMsgStr + "|r") : "|cffFF0000Never|r"));
    handler->SendSysMessage(Acore::StringFormat("> Muted: {} || Mute Time: {}", isMuted ? "|cffFF0000Yes|r" : "|cff4CFF00No|r", isMuted ? ("|cffFF0000" + secsToTimeString(muteTime - GameTime::GetGameTime().count(), true) + "|r") : "|cff4CFF000s|r"));
    handler->SendSysMessage(Acore::StringFormat("> Total Mutes: {}{}|r || Banned: {}", totalMutes > 0 ? "|cffFF0000" : "|cff4CFF00", totalMutes, isBanned ? "|cffFF0000Yes|r" : "|cff4CFF00No|r"));
}